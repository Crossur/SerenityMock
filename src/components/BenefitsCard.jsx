...
return (
  <div style={{ perspective: "1500px" }} className={`px-4 w-full h-[22rem] cursor-pointer group hover:scale-98 duration-300`} onClick={() => setFlipped(prev => !prev)}>
    <div className={`relative w-full h-full transition-transform duration-700 transform-style-preserve-3d rounded-xl ${flipped ? "rotate-y-180" : ""}`}>
      
      {/* Front Side */}
      <div className="absolute inset-0 backface-hidden rounded-xl shadow-xl hover:shadow-2xl transition group">
        <img src={imageUrl} className="h-full w-full object-cover rounded-xl grayscale group-hover:grayscale-0 transition" alt={header} />
